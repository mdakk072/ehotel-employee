<template>
  <div class="login-container" style="background-image: url('https://i0.wp.com/demishia.com/wp-content/uploads/2018/04/admin-login-background-images-8.jpg?ssl=1')">
    <form class="form-signin" @submit.prevent="submitForm">
      ...
      <button class="btn  btn-outline-light m-2" @click="getRandomAcc"  type="button">Compte Random</button>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog  modal-dialog-scrollable">
    ...
    <form @submit.prevent="createEmploye" id="registerForm">
      <div class="form-group">
        <label for="nas">NAS de l'employé :</label>
      ...
      <div class="form-group">
        <label for="idhotel">ID de l'hôtel :</label>
    
      <select v-model="selectedChaine" style="background: rgba(255,255,255,0.8);" class="form-select m-1">
  <option v-for="chaine in chainehotelieres" :key="chaine.idchaine" :value="chaine.idchaine">{{ chaine.nom }}</option>
</select>
  ...
  .then(response => {
    ...
    if (response.status === 200) {
      ...
      response.json().then(data => {
        ...
        this.$router.push('/dashboard/'+data.token);
      });
...
,
getRandomAcc() {
  fetch('http://localhost:3000/randomEmployee')
...
createEmploye() {
      const employe = {
        ...
        idhotel: this.selectedChaine
      };
      console.log(employe)
      // Ajouter le code pour insérer l'employé dans la base de données ici
    }
